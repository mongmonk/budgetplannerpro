...
import { Button, LogOutIcon, PaletteIcon, SunIcon, MoonIcon, CheckCircleIcon, PlusIcon, TrashIcon, EditIcon, Modal, CurrencyInput, HelpCircleIcon } from '../ui';
...
export const MobileSettings: React.FC<{ onLogout: () => void; user: User | null }> = ({ onLogout, user }) => {
    const { state, setState, updateUserSettings, colorScheme, setColorScheme, activeAccent, setIsHelpModalOpen } = useAppContext();
                    ...
                    <>
                        ...
                        <MobileCard>
                            <div className="flex items-center gap-2 mb-4">
                                <HelpCircleIcon className="w-5 h-5 text-primary-500" />
                                <h3 className="font-bold dark:text-white">Bantuan</h3>
                            </div>
                            <Button onClick={() => setIsHelpModalOpen(true)} className="w-full rounded-2xl py-3 flex items-center justify-center gap-2" variant="secondary">

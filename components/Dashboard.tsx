...
export const Dashboard: React.FC = () => {
    ...
    const totalAssets = useMemo(() => (state.wallets || []).reduce((s, w) => s + w.balance, 0), [state.wallets]);
        ...
        <div className="space-y-6 pb-16 md:pb-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-6">
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-white flex items-center gap-2"><ReceiptIcon className="w-6 h-6 text-primary-500"/>Dompet & Akun</h2>
                    <Card>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {state.wallets && state.wallets.length > 0 ? state.wallets.map(w => (
                                <div key={w.id} className="p-4 bg-slate-100 dark:bg-slate-700/50 rounded-xl flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-lg bg-primary-500/20 flex items-center justify-center text-2xl">{w.icon || '💰'}</div>
                                    <div className="flex-1 min-w-0">
                                        <p className="font-semibold truncate">{w.name}</p>
                                        <p className="text-xs text-slate-500 dark:text-slate-400">{w.type}</p>
                                        <p className="text-sm font-bold text-primary-600 dark:text-primary-400">{formatCurrency(w.balance)}</p>
                                    </div>
                                </div>
                            )) : <p className="text-center text-slate-500 py-4 col-span-2">Belum ada dompet.</p>}
                        </div>
                    </Card>
                </div>
                <div className="space-y-6">
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-white flex items-center gap-2"><TargetIcon className="w-6 h-6 text-primary-500"/>Progres Target Finansial</h2>
                    {goals.length > 0 ? <div className="grid grid-cols-1 gap-4">{goals.map(g => <Card key={g.id}><h3 className="font-semibold">{g.name}</h3><div className="flex justify-between items-baseline mt-2 mb-1"><span className="text-sm font-medium text-primary-600 dark:text-primary-400">{formatCurrency(g.currentAmount)}</span><span className="text-xs text-slate-500 dark:text-slate-400">Target: {formatCurrency(g.targetAmount)}</span></div><ProgressBar value={g.currentAmount} max={g.targetAmount} /></Card>)}</div> : <Card><p className="text-center text-slate-500 dark:text-slate-400">Anda belum punya target.</p></Card>}
                </div>
            </div>
